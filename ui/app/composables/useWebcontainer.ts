...
export const useWebcontainerStore = defineStore("webcontainer", () => {
  ...
  const status = ref<
    "idle" | "error" | "install" | "analyse" | "graph" | "finish"
  >("idle");
  ...
  async function spawn(command: string, args: string[] = []) {
    if (currentProcess.value) throw new Error("A process is already running");
    const process = await webcontainerInstance.value!.spawn(command, args);
    currentProcess.value = process;
    return process.exit.then((r) => {
      if (currentProcess.value === process) currentProcess.value = undefined;
      return r;
    });
  }
  ...
  async function cleanProgress() {
    const cleanProgress = await spawn("rm", [
      "-rf",
