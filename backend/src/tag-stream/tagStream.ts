...
export class TagStream {
  ...
  async giveUnitsToRepoContributors(
    ...
    if (!this.signer) await this.connectWallet();
    return this.contract.giveUnitsToRepoContributors(
      repoId,
      developerIds,
      units,
    );
  ...
  async flowDistributeToRepo(
    ...
    if (
      (await underlyingToken.allowance(address, TagStream.address)) <
      amountInWei
    ) {
    ...
    return this.contract.flowDistributeToRepo(
      repoId,
      amountInWei,
      durationInSeconds,
    );
